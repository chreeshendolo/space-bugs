...
AI_SCRIPTS[AI_STATE.ENGAGED] = function (entity) {
	...
	const engaged_radius = entity.vision_radius * 4;
	...
	if (distance > engaged_radius) { // disengage
		...
		entity.ai_state = AI_STATE.IDLE;
		return;
	} else if (!lineOfSight || distance > engaged_radius * 0.7) { // move closer
		...
		if (entity.destination === undefined) {
			const rectangle = {position: entity.position, width: engaged_radius, height: engaged_radius};
	...
	} else if (distance < engaged_radius * 0.3){ // move away
