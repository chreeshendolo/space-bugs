...
music.load = function (paths, callback) {
	...
	for (let i = 0; i < paths.length; i++) {
		...
		for (let i = 0; i < TRACK_COPIES; i++) {
			...
			sound.copies.push(musicTrack);

			musicTrack.addEventListener('canplaythrough', (event) => {
				if (++total < paths.length * TRACK_COPIES) { return; } // todo: update this value
				callback();
			});
		}
	}
...
music.update = function (deltaTime) {
	if (this.isPlaying) {
		...
		if (this.time >= MS_PER_CHORD / 1000) {
			this.time = 0;

			this.chordIndex++;
			if (this.chordIndex === 4) { this.chordIndex = 0; }
