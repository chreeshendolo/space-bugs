...
rayCasting.hasLineOfSight = function (positionA, positionB) {
	const ray = [positionA, positionB];
	const wallSegments = this.getWallSegments(MAP_DATA);

	for (let i = 0; i < wallSegments.length; i++) {
		const [A, B] = wallSegments[i];
		const segment = [
			[A.x * grid.unit, A.y * grid.unit],
			[B.x * grid.unit, B.y * grid.unit],
		];

		if (this.doesIntersect(ray, segment)) { return false; }
	}

	return true;
}

rayCasting.isIndexInLineOfSight = function (position, index, indexes) {

	const vertices = grid.getIndexVerticies(index);
	for (let i = 0; i < vertices.length; i++) {
		const ray = [position, vertices[i]];

		for (let ii = 0; ii < indexes.length; ii++) {
			const _index = indexes[ii];
			if (index === _index) { continue; }

			const _verticies = grid.getIndexVerticies(_index);
			for (let iii = 0; iii < _verticies.length; iii++) {
				const segment = [_verticies[iii], _verticies[(iii + 1) % _verticies.length]];
				if (this.doesIntersect(ray, segment)) { return true; }
			}
		}
...
rayCasting.doesIntersect = function (ray, segment) {
	const [A, B] = segment;
	const [C, D] = ray;

	const denominator = (D[0] - C[0]) * (B[1] - A[1]) - (B[0] - A[0]) * (D[1] - C[1]);

	const r = ((B[0] - A[0]) * (C[1] - A[1]) - (C[0] - A[0]) * (B[1] - A[1])) / denominator;
	if (r < 0 || r > 1) { return false; }

	const s = ((A[0] - C[0]) * (D[1] - C[1]) - (D[0] - C[0]) * (A[1] - C[1])) / denominator;
...
rayCasting.getRays = function (player) {
	...
	const vertexIndices = this.getVertexIndices(MAP_DATA);
